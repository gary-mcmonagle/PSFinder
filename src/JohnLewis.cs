...
namespace PSFinder.src
{
    public class JohnLewis: Store
    {
        ...
        async private Task<string> getPage()
        {
            Console.WriteLine("Getting John Lewis");
            ...
            var resultTwo = await url.GetJsonAsync<GraphQLResponse>();
            Console.WriteLine(resultTwo.data.productsV2[0].variants[0].title);
            return "";

            Console.WriteLine("Got John Lewis");
        ...
        async private Task<bool> checkPage()
        {
            ...
            var availability = htmlDoc.DocumentNode.SelectSingleNode("//*[@id=\"button--add-to-basket-out-of-stock\"]");
